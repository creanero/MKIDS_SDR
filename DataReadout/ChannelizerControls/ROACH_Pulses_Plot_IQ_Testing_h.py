...
class AppForm(QMainWindow):
	...
	phaseraw = []
	    ...
	    Iraw0.append(struct.unpack('>h', bin_data_I[m:m+2])[0])
	    Iraw1.append(struct.unpack('>h', bin_data_I[m+1:m+3])[0])
	    Iraw2.append(struct.unpack('>h', bin_data_I[m+2:m+4])[0])
	    Iraw3.append(struct.unpack('>h', bin_data_I[m+3:m+5])[0])

	    Iraw4.append(struct.unpack('>h', bin_data_I[2*m:2*m+2])[0])
	    Iraw5.append(struct.unpack('>h', bin_data_I[2*m+1:2*m+3])[0])
	    Iraw6.append(struct.unpack('>h', bin_data_I[2*m+2:2*m+4])[0])
	    Iraw7.append(struct.unpack('>h', bin_data_I[2*m+3:2*m+5])[0])
	    
	    Iraw8.append(struct.unpack('>h', bin_data_I[3*m:3*m+2])[0])
	    Iraw9.append(struct.unpack('>h', bin_data_I[3*m+1:3*m+3])[0])
	    Iraw10.append(struct.unpack('>h', bin_data_I[3*m+2:3*m+4])[0])
	    Iraw11.append(struct.unpack('>h', bin_data_I[3*m+3:3*m+5])[0])
	    
	    Iraw12.append(struct.unpack('>h', bin_data_I[4*m:4*m+2])[0])
	    Iraw13.append(struct.unpack('>h', bin_data_I[4*m+1:4*m+3])[0])
	    Iraw14.append(struct.unpack('>h', bin_data_I[4*m+2:4*m+4])[0])
	...
	self.axes0.clear()
        ...
        if steps <= 1000:
       	    self.axes0.plot(Iraw13, 'b.', markersize=1) #changed to plot I0 versus time
	    self.axes1.plot(Iraw14, 'b.', markersize=1) #changed to plot I0 versus time
