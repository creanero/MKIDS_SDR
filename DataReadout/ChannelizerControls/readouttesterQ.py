...
bin_data_Q = roach.read('conv_phase_snapQ_bram', 4*L)
...
for m in range(L):
	Iraw.append(struct.unpack('>h', bin_data_Q[4*m+2:4*m+4])[0])
		
	
	print "m = ", m, "Iraw = ", struct.unpack('>h', bin_data_Q[4*m+2:4*m+4])[0]
