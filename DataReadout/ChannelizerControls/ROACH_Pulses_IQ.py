...
#from bitstring import BitArray commenting this to see if stops error
...
class AppForm(QMainWindow):
	...
	print 'programming roach...'
        self.roach.progdev('snap_raw_iq_20211204_2021_Dec_05_1816.bof')
	...
	ch_we = 27
        self.roach.write_int('conv_phase_ch_we_IQ', ch_we)
	...
	bin_data_Q = ''
        ...
        for n in range(steps):
        	self.roach.write_int('conv_phase_startSnapI', 0)
		self.roach.write_int('conv_phase_startSnapQ', 0)
		...
		self.roach.write_int('conv_phase_startSnapI', 1)
		self.roach.write_int('conv_phase_startSnapQ', 1)
		time.sleep(0.001)
	...
	Qraw = []
        ...
        phase = 360 * (numpy.arctan2(Qraw, Iraw)) / (2*numpy.pi)
	...
	while self.contsnapStatus == 'off':
		...
		median_phase = numpy.median(phase)	
		...
		for i in range(numberofaverages):
			phase_means[i] = numpy.mean(phase[(averagelength*(i+1) - averagelength):averagelength*(i+1)])
		...
		while bob < len(phase):	
			...
			#pulsenumberarray = pulsenumberarray.tolist()
			bob_array = phase[bob-500:bob+1500]
			bob_array_I = Iraw[bob-500:bob+1500]
			bob_array_Q = Qraw[bob-500:bob+1500]
			...
			if abs(phase_means[whichmean] - phase[bob]) > phase_threshold: 
				#print 'QDR Phase Value: ', qdr_phase_values[bob]
				george = george + 1			
				...
				pulsefilename = saveDir + 'pulses_'+datetime.utcnow().strftime('%Y-%m-%d_%H%M%S%f')[:-3] +'.txt' 
				...
				if total_pulses == 0:
					...
					finalIarray.extend(bob_array_I)
					finalQarray.extend(bob_array_Q)
		...
		print 'Iteration: ', failsafe, ', Counts per iteration: ', countrate

		self.axes1.clear()
		self.axes1.plot(bob_array,'b.')
		time.sleep(1)
		self.axes0.clear()
		self.axes0.plot(bob_array_I, bob_array_Q,'b.')
		time.sleep(1)
	...
	if total_pulses != 0:	
        	savearray = [pulsenumberarray, finalphasearray, finalIarray, finalQarray]
		numpy.savetxt(pulsefilename,numpy.column_stack(savearray),fmt='%i ' '%.2f ' '%i ' '%i')
	...
	label_seconds = QLabel('* 1 sec')
        ...
        self.textbox_snapSteps = QLineEdit('1')
        self.textbox_snapSteps.setMaximumWidth(50)
        label_snapSteps = QLabel('* 30 msec')
	...
	# normalisation for noise measurements. This is the signal voltage (in micro Volts [uV]) that is input to each of the ADCs.
        ...
        label_contsnapSteps = QLabel('* 30 msec')
