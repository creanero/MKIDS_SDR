...
class AppForm(QMainWindow):
	...
	self.axes2.clear()
	    ...
	    self.axes0.set_xlabel('Freq [Hz]')
            self.axes0.set_ylabel('S21 (Magnitude)')
        except ValueError:
            pass
	...
	self.axes2.set_ylabel('S21 (Phase)')

	self.axes3.plot(I, Q, '.-', self.iq_centers.real[0:self.N_freqs], self.iq_centers.imag[0:self.N_freqs], '.', self.I_on_res, self.Q_on_res, '.')
	self.axes3.set_xlabel('I (In-Phase)')
	self.axes3.set_ylabel('Q (In-Quadrature)')
	...
	self.axes3.plot(self.I[ch], self.Q[ch], '.-', self.iq_centers.real[ch], self.iq_centers.imag[ch], '.', self.I_on_res[ch], self.Q_on_res[ch], '.')
	self.axes3.set_xlabel('I (In-Phase)')
	self.axes3.set_ylabel('Q (In-Quadrature)')
        if self.last_I != None and self.showPrevious:
...
#        elif roachNo == 7:
#            roachIP = 6
        self.textbox_roachIP = QLineEdit('192.168.4.10')
        ...
        # Power sweep range. 
        self.textbox_powerSweepStart = QLineEdit('0')
        self.textbox_powerSweepStart.setMaximumWidth(50)
        label_powerSweepStart = QLabel('Start atten.:')
        self.textbox_powerSweepStop = QLineEdit('0')
        self.textbox_powerSweepStop.setMaximumWidth(50)
        label_powerSweepStop = QLabel('Stop atten:')

        # Save directory
        self.textbox_saveDir = QLineEdit('/home/cbracken/Desktop/SDR-master/DataReadout/ChannelizerControls/LUT')
        ...
        self.textbox_freqFile = QLineEdit('/home/cbracken/Desktop/SDR-master/DataReadout/ChannelizerControls/LUT/freqTest250.txt')
        ...
        self.textbox_customScale = QLineEdit('1')
        ...
        label_freqAutoLimit = QLabel('Max Freq Jump (Hz):')
