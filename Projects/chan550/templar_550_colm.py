...
class AppForm(QMainWindow):
	...
	self.setStyleSheet("QLabel {font: 12pt Comic Sans MS}")
        self.create_menu()
	...
	#self.numpy.savetxt('Spec_freq_new.out', f_span, delimiter=' ')
    	#self.numpy.savetxt('Spec_power_new.out', (I**2 + Q**2)**.5, delimiter=' ')
    ...
    def getADCData(self):
    	...
    	a_0=struct.unpack(self.roach.read('bram0',2048))
    	a_1=struct.unpack(self.roach.read('bram1',2048))
    ...
    def plot_spectrum(self):
    	...
    	epsilon = 0.1953125
    	XX = numpy.arange(0,2048)
    	X2 = epsilon*XX


    	matplotlib.pylab.plot(X2,interleave_a)
    	...
    	matplotlib.pylab.ylim(0.5e5, 5.67e5)
    	matplotlib.pylab.grid()
    	matplotlib.pylab.xlabel('Frequency [MHz]')
    	matplotlib.pylab.xlim(0,10)
    	fig.canvas.draw()
    	fig.canvas.manager.window.after(100, plot_spectrum)

    	numpy.savetxt('testerfreq.out', X2, delimiter=',')
    	numpy.savetxt('testerrpower.out', interleave_a, delimiter=',')
    ...
    def loadLUTs(self):
        ...
        freq_residuals = select_bins(freqs_dds)
	...
	#def savevalues(self):
        #self.fdata[self.resnum] = self.N_freqs
        #self.SdataList[self.resnum] = self.atten
    ...
    def channelIncDown(self):
        ...
        self.axes1.clear()
        self.axes0.semilogy(self.f_span[ch], (self.I[ch]**2 + self.Q[ch]**2)**.5, '.-')
        self.axes1.plot(self.f_span[ch], math.atan2(self.Q[ch],self.I[ch]), '.-')
        self.canvas.draw()
    ...
    def create_main_frame(self):
        ...
        # Save directory
        self.textbox_saveDir = QLineEdit('/opt/software/colin1')
        ...
        self.textbox_freqFile = QLineEdit('/opt/software/colin1/LUT/freqTest2.txt')
